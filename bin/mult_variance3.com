...
set incr = `echo ${incr} ${nproc} | awk '{print int($1/$2+1)}'`
...
if (`echo ${cluster_type} | tr '[a-z]' '[A-Z]'` == "CONDOR") then
  if ( $sym == "H" || $sym == "HP" ) then
    ...
    echo "executable = ${bin_dir}/mult_reconstruct_v2.com"  > $SCRATCH/condor.com
  endif
...
while ( $nc <= $nclass )
  ...
  while ( $islast == 0 )
    ...
    if ( $lastn == $last ) set islast = 1

    @ npart++
    echo "Cycle "$start": reconstructing bootstrap volume $nc, particles $firstn to $lastn on "`date` >> frealign.log
    ...
    if (`echo ${cluster_type} | tr '[a-z]' '[A-Z]'` == "SGE") then
      if ( $sym == "H" || $sym == "HP" ) then
        set pid = `qsub $stn -l mem_free=${mem_small}G -cwd -r y -V -e $SCRATCH/stderr -o $SCRATCH/stderr -N r${nc}_${firstn} ${bin_dir}/mult_hreconstruct_v.com $firstn $lastn $start $nc | awk '{print $3}'`
      else
        set pid = `qsub $stn -l mem_free=${mem_small}G -cwd -r y -V -e $SCRATCH/stderr -o $SCRATCH/stderr -N r${nc}_${firstn} ${bin_dir}/mult_reconstruct_v2.com $firstn $lastn $start $nc | awk '{print $3}'`
      endif
    else if (`echo ${cluster_type} | tr '[a-z]' '[A-Z]'` == "SLURM") then
      if ( $sym == "H" || $sym == "HP" ) then
        set pid = `sbatch $stn -D $working_directory -J r${nc}_${firstn} -o $SCRATCH/stderr ${bin_dir}/mult_hreconstruct_v.com $firstn $lastn $start $nc | awk '{print $4}'`
      else
        set pid = `sbatch $stn -D $working_directory -J r${nc}_${firstn} -o $SCRATCH/stderr ${bin_dir}/mult_reconstruct_v.com $firstn $lastn $start $nc | awk '{print $4}'`
      endif
    else if (`echo ${cluster_type} | tr '[a-z]' '[A-Z]'` == "LSF") then 
      if ( $sym == "H" || $sym == "HP" ) then
#        set pid = `bsub -W 12:00 -q short -R "select[model!=Opteron2216]" -R "rusage[mem=${mem_small}]" -cwd  "$working_directory" -r -e $SCRATCH/stderr -o $SCRATCH/stderr ${bin_dir}/mult_hreconstruct_v.com $firstn $lastn $start $nc | awk -F\< '{print $2}' | awk -F\> '{print $1}'`
        set pid = `bsub $stn -cwd "$working_directory" -r -e $SCRATCH/stderr -o $SCRATCH/stderr ${bin_dir}/mult_hreconstruct_v.com $firstn $lastn $start $nc | awk -F\< '{print $2}' | awk -F\> '{print $1}'`
      else
#        set pid = `bsub -W 12:00 -q short -R "select[model!=Opteron2216]" -R "rusage[mem=${mem_small}]" -cwd  "$working_directory" -r -e $SCRATCH/stderr -o $SCRATCH/stderr ${bin_dir}/mult_reconstruct_v2.com $firstn $lastn $start $nc | awk -F\< '{print $2}' | awk -F\> '{print $1}'`
        set pid = `bsub $stn -cwd "$working_directory" -r -e $SCRATCH/stderr -o $SCRATCH/stderr ${bin_dir}/mult_reconstruct_v2.com $firstn $lastn $start $nc | awk -F\< '{print $2}' | awk -F\> '{print $1}'`
      endif
      ...
      if ( $sym == "H" || $sym == "HP" ) then
        ...
        echo ${bin_dir}/mult_reconstruct_v2.com $firstn $lastn $start $nc \& >> $SCRATCH/slurm.com
      endif
      ...
      if ( $sym == "H" || $sym == "HP" ) then
        ...
        echo ${bin_dir}/mult_reconstruct_v2.com $firstn $lastn $start $nc >> $SCRATCH/pbs_${firstn}_${nc}.com
      endif
      ...
      if ( $sym == "H" || $sym == "HP" ) then
        ...
        ssh $rh "cd $working_directory; ${bin_dir}/mult_reconstruct_v2.com $firstn $lastn $start $nc" &
      endif
      set pid = $!
    else
      if ( $sym == "H" || $sym == "HP" ) then
        ...
        ${bin_dir}/mult_reconstruct_v2.com $firstn $lastn $start $nc &
      endif
      set pid = $!
    endif
...
# while ( $nc <= $nclass )
  ...
  #  sleep 5
    while ( $firstn <= $last )
      ...
      echo "Cycle "$start": reconstruction of bootstrap volume $nc, particles $firstn to $lastn, finished "`date` >> frealign.log
      if (`echo ${cluster_type} | tr '[a-z]' '[A-Z]'` != "STAMPEDE" ) then
...
#while ( $nc <= $nclass )
  ...
  if (`echo ${cluster_type} | tr '[a-z]' '[A-Z]'` == "SGE") then
    set pid = `qsub $stn -l mem_free=${mem_small}G -cwd -r y -V -e $SCRATCH/stderr -o $SCRATCH/stderr -N m${nc} ${bin_dir}/merge_3d.com ${nc} | awk '{print $3}'`
  else if (`echo ${cluster_type} | tr '[a-z]' '[A-Z]'` == "SLURM") then
    set pid = `sbatch $stn -D $working_directory -J m${nc} -o $SCRATCH/stderr ${bin_dir}/merge_3d.com ${nc} | awk '{print $4}'`
  else if (`echo ${cluster_type} | tr '[a-z]' '[A-Z]'` == "LSF") then 
#    set pid = `bsub -W 12:00 -q short -R "select[model!=Opteron2216]" -R "rusage[mem=${mem_small}]" -cwd  "$working_directory" -r -e $SCRATCH/stderr -o $SCRATCH/stderr ${bin_dir}/merge_3d.com ${nc} | awk -F\< '{print $2}' | awk -F\> '{print $1}'`
    set pid = `bsub $stn -cwd "$working_directory" -r -e $SCRATCH/stderr -o $SCRATCH/stderr ${bin_dir}/merge_3d.com ${nc} | awk -F\< '{print $2}' | awk -F\> '{print $1}'`
  else if (`echo ${cluster_type} | tr '[a-z]' '[A-Z]'` == "STAMPEDE") then 
    echo \#\!\/bin\/csh -f > $SCRATCH/slurm.com
...
checkdonem:
  ...
  cat $SCRATCH/${data_input}_${start}_r${nc}.res >> $SCRATCH/${data_input}_${start}_r${nc}.par
  mv $SCRATCH/${data_input}_${start}_r${nc}.${extension} $SCRATCH/${data_input}_${start}_r${nc}.${extension}

  set form = `${bin_dir}/fheader.exe ${raw_images}.${extension} | grep --binary-files=text Opening | awk '{print $2}'`
  if ( $form == "IMAGIC" ) mv $SCRATCH/${data_input}_${start}_r${nc}.img $SCRATCH/${data_input}_${start}_r${nc}.img
...
echo "Cycle "$start": reconstruction done." >> frealign.log
  ...
  if (`echo ${cluster_type} | tr '[a-z]' '[A-Z]'` == "SGE") then
    set pid = `qsub $stn -l mem_free=${mem_small}G -cwd -r y -V -e $SCRATCH/stderr -o $SCRATCH/stderr -N bootvar ${bin_dir}/bootvar.com | awk '{print $3}'`
  else if (`echo ${cluster_type} | tr '[a-z]' '[A-Z]'` == "SLURM") then
    set pid = `sbatch $stn -D $working_directory -J bootvar -o $SCRATCH/stderr ${bin_dir}/bootvar.com | awk '{print $4}'`
  else if (`echo ${cluster_type} | tr '[a-z]' '[A-Z]'` == "LSF") then 
#    set pid = `bsub -W 12:00 -q short -R "select[model!=Opteron2216]" -R "rusage[mem=${mem_small}]" -cwd  "$working_directory" -r -e $SCRATCH/stderr -o $SCRATCH/stderr ${bin_dir}/bootvar.com | awk -F\< '{print $2}' | awk -F\> '{print $1}'`
    set pid = `bsub $stn -cwd "$working_directory" -r -e $SCRATCH/stderr -o $SCRATCH/stderr ${bin_dir}/bootvar.com | awk -F\< '{print $2}' | awk -F\> '{print $1}'`
  else if (`echo ${cluster_type} | tr '[a-z]' '[A-Z]'` == "STAMPEDE") then 
    echo \#\!\/bin\/csh -f > $SCRATCH/slurm.com
    ...
    echo "executable = ${bin_dir}/bootvar.com"   > $SCRATCH/condor.com
    echo "universe = vanilla"                   >> $SCRATCH/condor.com
    echo "request_memory = ${mem_small} GB"     >> $SCRATCH/condor.com
    echo "output = $SCRATCH/condor.out" >> $SCRATCH/condor.com
    echo "error = $SCRATCH/condor.err" >> $SCRATCH/condor.com
