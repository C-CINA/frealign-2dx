...
loop_recovery:
    ...
    if (${shiftiter} == 0) then
      if (${angleiter} == 0) then
        if (`echo ${cluster_type} | tr '[a-z]' '[A-Z]'` == "SGE") then
          if ( $sym == "H" || $sym == "HP" ) then
            set pid = `qsub $stn -l mem_free=${mem_big}G -cwd -r y -V -e $SCRATCH/stderr -o $SCRATCH/stderr -N n${nc}_${fst} ${bin_dir}/mult_hrefine_n.com $fst $lastn $start $nc $mode $pmask $$ | awk '{print $3}'`
          else
            set pid = `qsub $stn -l mem_free=${mem_big}G -cwd -r y -V -e $SCRATCH/stderr -o $SCRATCH/stderr -N n${nc}_${fst} ${bin_dir}/mult_refine_n.com $fst $lastn $start $nc $mode $pmask $$ | awk '{print $3}'`
          endif
        else if (`echo ${cluster_type} | tr '[a-z]' '[A-Z]'` == "SLURM") then
          if ( $sym == "H" || $sym == "HP" ) then
            set pid = `sbatch $stn -D $working_directory -J n${nc}_${fst} -o $SCRATCH/stderr ${bin_dir}/mult_hrefine_n.com $fst $lastn $start $nc $mode $pmask $$ | awk '{print $4}'`
          else
            set pid = `sbatch $stn -D $working_directory -J n${nc}_${fst} -o $SCRATCH/stderr ${bin_dir}/mult_refine_n.com $fst $lastn $start $nc $mode $pmask $$ | awk '{print $4}'`
          endif
        else if (`echo ${cluster_type} | tr '[a-z]' '[A-Z]'` == "LSF") then 
          if ( $sym == "H" || $sym == "HP" ) then
#            set pid = `bsub -W 12:00 -q short -R "select[model!=Opteron2216]" -R "rusage[mem=${mem_big}]" -cwd  "$working_directory" -r -e $SCRATCH/stderr -o $SCRATCH/stderr ${bin_dir}/mult_hrefine_n.com $fst $lastn $start $nc $mode $pmask $$ | awk -F\< '{print $2}' | awk -F\> '{print $1}'`
            set pid = `bsub $stn -cwd "$working_directory" -r -e $SCRATCH/stderr -o $SCRATCH/stderr ${bin_dir}/mult_hrefine_n.com $fst $lastn $start $nc $mode $pmask $$ | awk -F\< '{print $2}' | awk -F\> '{print $1}'`
          else
#            set pid = `bsub -W 12:00 -q short -R "select[model!=Opteron2216]" -R "rusage[mem=${mem_big}]" -cwd  "$working_directory" -r -e $SCRATCH/stderr -o $SCRATCH/stderr ${bin_dir}/mult_refine_n.com $fst $lastn $start $nc $mode $pmask $$ | awk -F\< '{print $2}' | awk -F\> '{print $1}'`
            set pid = `bsub $stn -cwd "$working_directory" -r -e $SCRATCH/stderr -o $SCRATCH/stderr ${bin_dir}/mult_refine_n.com $fst $lastn $start $nc $mode $pmask $$ | awk -F\< '{print $2}' | awk -F\> '{print $1}'`
          endif
        else if (`echo ${cluster_type} | tr '[a-z]' '[A-Z]'` == "STAMPEDE") then
          if ( $sym == "H" || $sym == "HP" ) then
        ...
        if (`echo ${cluster_type} | tr '[a-z]' '[A-Z]'` == "SGE") then
          if ( $sym == "H" || $sym == "HP" ) then
            ...
            set pid = `qsub $stn -l mem_free=${mem_big}G -cwd -r y -V -e $SCRATCH/stderr -o $SCRATCH/stderr -N n${nc}_${fst} ${bin_dir}/mult_refine_n.com $fst $lastn $start $nc 1 $pshift $$ | awk '{print $3}'`
          endif
        else if (`echo ${cluster_type} | tr '[a-z]' '[A-Z]'` == "SLURM") then
          if ( $sym == "H" || $sym == "HP" ) then
            ...
            set pid = `sbatch $stn -D $working_directory -J n${nc}_${fst} -o $SCRATCH/stderr ${bin_dir}/mult_refine_n.com $fst $lastn $start $nc 1 $pshift $$ | awk '{print $4}'`
          endif
        else if (`echo ${cluster_type} | tr '[a-z]' '[A-Z]'` == "LSF") then 
          if ( $sym == "H" || $sym == "HP" ) then
...
#            set pid = `bsub -W 12:00 -q short -R "select[model!=Opteron2216]" -R "rusage[mem=${mem_big}]" -cwd  "$working_directory" -r -e $SCRATCH/stderr -o $SCRATCH/stderr ${bin_dir}/mult_refine_n.com $fst $lastn $start $nc 1 $pshift $$ | awk -F\< '{print $2}' | awk -F\> '{print $1}'`
            set pid = `bsub $stn -cwd "$working_directory" -r -e $SCRATCH/stderr -o $SCRATCH/stderr ${bin_dir}/mult_refine_n.com $fst $lastn $start $nc 1 $pshift $$ | awk -F\< '{print $2}' | awk -F\> '{print $1}'`
          endif
        else if (`echo ${cluster_type} | tr '[a-z]' '[A-Z]'` == "STAMPEDE") then
          if ( $sym == "H" || $sym == "HP" ) then
      ...
      if (${angleiter} == 0) then
        if (`echo ${cluster_type} | tr '[a-z]' '[A-Z]'` == "SGE") then
          if ( $sym == "H" || $sym == "HP" ) then
            ...
            set pid = `qsub $stn -l mem_free=${mem_big}G -cwd -r y -V -e $SCRATCH/stderr -o $SCRATCH/stderr -N n${nc}_${fst} ${bin_dir}/mult_refine_n.com $fst $lastn $start $nc 1 $pangle $$ | awk '{print $3}'`
          endif
        else if (`echo ${cluster_type} | tr '[a-z]' '[A-Z]'` == "SLURM") then
          if ( $sym == "H" || $sym == "HP" ) then
            ...
            set pid = `sbatch $stn -D $working_directory -J n${nc}_${fst} -o $SCRATCH/stderr ${bin_dir}/mult_refine_n.com $fst $lastn $start $nc 1 $pangle $$ | awk '{print $4}'`
          endif
        else if (`echo ${cluster_type} | tr '[a-z]' '[A-Z]'` == "LSF") then 
          if ( $sym == "H" || $sym == "HP" ) then
...
#            set pid = `bsub -W 12:00 -q short -R "select[model!=Opteron2216]" -R "rusage[mem=${mem_big}]" -cwd  "$working_directory" -r -e $SCRATCH/stderr -o $SCRATCH/stderr ${bin_dir}/mult_refine_n.com $fst $lastn $start $nc 1 $pangle $$ | awk -F\< '{print $2}' | awk -F\> '{print $1}'`
            set pid = `bsub $stn -cwd "$working_directory" -r -e $SCRATCH/stderr -o $SCRATCH/stderr ${bin_dir}/mult_refine_n.com $fst $lastn $start $nc 1 $pangle $$ | awk -F\< '{print $2}' | awk -F\> '{print $1}'`
          endif
        else if (`echo ${cluster_type} | tr '[a-z]' '[A-Z]'` == "STAMPEDE") then
          if ( $sym == "H" || $sym == "HP" ) then
        ...
        if (`echo ${cluster_type} | tr '[a-z]' '[A-Z]'` == "SGE") then
          if ( $sym == "H" || $sym == "HP" ) then
            ...
            set pid = `qsub $stn -l mem_free=${mem_big}G -cwd -r y -V -e $SCRATCH/stderr -o $SCRATCH/stderr -N n${nc}_${fst} ${bin_dir}/mult_refine_n.com $fst $lastn $start $nc 1 0 0 0 0 0 $$ | awk '{print $3}'`
          endif
        else if (`echo ${cluster_type} | tr '[a-z]' '[A-Z]'` == "SLURM") then
          if ( $sym == "H" || $sym == "HP" ) then
            ...
            set pid = `sbatch $stn -D $working_directory -J n${nc}_${fst} -o $SCRATCH/stderr ${bin_dir}/mult_refine_n.com $fst $lastn $start $nc 1 0 0 0 0 0 $$ | awk '{print $4}'`
          endif
        else if (`echo ${cluster_type} | tr '[a-z]' '[A-Z]'` == "LSF") then 
          if ( $sym == "H" || $sym == "HP" ) then
...
#            set pid = `bsub -W 12:00 -q short -R "select[model!=Opteron2216]" -R "rusage[mem=${mem_big}]" -cwd  "$working_directory" -r -e $SCRATCH/stderr -o $SCRATCH/stderr ${bin_dir}/mult_refine_n.com $fst $lastn $start $nc 1 0 0 0 0 0 $$ | awk -F\< '{print $2}' | awk -F\> '{print $1}'`
            set pid = `bsub $stn -cwd "$working_directory" -r -e $SCRATCH/stderr -o $SCRATCH/stderr ${bin_dir}/mult_refine_n.com $fst $lastn $start $nc 1 0 0 0 0 0 $$ | awk -F\< '{print $2}' | awk -F\> '{print $1}'`
          endif
        else if (`echo ${cluster_type} | tr '[a-z]' '[A-Z]'` == "STAMPEDE") then
          if ( $sym == "H" || $sym == "HP" ) then
...
checkdone:
  ...
  while ( $firstn <= $last )
    ...
    grep -i --binary-files=text "Normal termination" $SCRATCH/${data_input}_mult_refine_n_r${nc}.log_${fst}_${lastn} >& /dev/null
    ...
    if ($status_par != 0 && $status_log == 0) then
      ...
      \rm $SCRATCH/${data_input}_mult_refine_n_r${nc}.log_${fst}_${lastn}  >& /dev/null
      goto checkdone
    endif
