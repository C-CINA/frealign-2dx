  ...
  while ( $nc <= $nclass )
    ...
    if (`echo ${cluster_type} | tr '[a-z]' '[A-Z]'` == "SGE") then
      if ( $sym == "H" || $sym == "HP" ) then
        ...
        set pid = `qsub $stn -l mem_free=${mem_small}G -cwd -r y -V -e $SCRATCH/stderr -o $SCRATCH/stderr -N r${nc}_${firstn} ${bin_dir}/mult_reconstruct_v.com $firstn $lastn $start $nc | awk '{print $3}'`
      endif
    else if (`echo ${cluster_type} | tr '[a-z]' '[A-Z]'` == "SLURM") then
      if ( $sym == "H" || $sym == "HP" ) then
        set pid = `sbatch $stn -D $working_directory -J r${nc}_${firstn} -o $SCRATCH/stderr ${bin_dir}/mult_hreconstruct_n.com $firstn $lastn $start $nc | awk '{print $4}'`
      else
        set pid = `sbatch $stn -D $working_directory -J r${nc}_${firstn} -o $SCRATCH/stderr ${bin_dir}/mult_reconstruct_n.com $firstn $lastn $start $nc | awk '{print $4}'`
      endif
...
#        set pid = `bsub -W 12:00 -q short -R "select[model!=Opteron2216]" -R "rusage[mem=${mem_small}]" -cwd  "$working_directory" -r -e $SCRATCH/stderr -o $SCRATCH/stderr ${bin_dir}/mult_reconstruct_v.com $firstn $lastn $start $nc | awk -F\< '{print $2}' | awk -F\> '{print $1}'`
        set pid = `bsub $stn -cwd "$working_directory" -r -e $SCRATCH/stderr -o $SCRATCH/stderr ${bin_dir}/mult_reconstruct_v.com $firstn $lastn $start $nc | awk -F\< '{print $2}' | awk -F\> '{print $1}'`
      endif
...
#  if ($status) goto checkdonem
  ...
  mv $SCRATCH/${data_input}_${start}_ave.${extension} ${working_directory}/${data_input}_${start}_r${4}.hkl
