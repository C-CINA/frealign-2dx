...
# Original mult_refine_auto_n script modified by Ricardo Righetto, University of Basel, 2017.
...
cd $SCRATCH
echo FREALIGN_auto-refinement_ is starting now `date` at `hostname`... r${4}.log_${1}_${2} >> stderr
...
set dang_old = $dang
if ( $mode != 1 ) set dang = 0.0
# echo $dang >> $working_directory/frealign.log
# echo $dang_old >> $working_directory/frealign.log
...
set usexstd = ${12}
# echo $usexstd >> $working_directory/frealign.log
if ( $usexstd ) then
  set XSTD  = `grep XSTD mparameters_run | awk '{print $2}'`
else
  set XSTD = 0.0
endif
# echo $XSTD >> $working_directory/frealign.log
...
# echo $mode
if ( $mode != 1 ) then 
  # echo "Cycle ${start}: Angular step was changed from $dang_old to $dang." >> $working_directory/frealign.log
  echo "Cycle ${start}: Angular step was changed from $dang_old to $dang."
endif
...
# echo "NODE SCRATCH IS: ${NODE_SCRATCH}" >> stderr
...
#
time ${bin_dir}/frealign_v9.exe << eot >& ${data_input}_mult_refine_auto_n_r${4}.log_${1}_${2}
