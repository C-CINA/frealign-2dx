...
cd $SCRATCH
#
echo FREALIGN_reconstruction_ is starting now `date`... r${4}.log_${1}_${2} >> stderr
...
#echo 'FREALIGN IS STARTING NOW!' >> ${data_input}_mult_reconstruct_r${4}_n${1}.start
set raw_images	= `grep raw_images_rec mparameters_run | awk '{print $2}'`
...
echo FREALIGN_reconstruction_ is starting now `date` at `hostname`... r${4}.log_${1}_${2} >> stderr
if ( $extension == "" ) then
...
#
#echo 'FREALIGN IS STARTING NOW!' >> ${data_input}_mult_reconstruct_r${4}_n${1}.start
time ${bin_dir}/frealign_v9${m}.exe << eot >>& ${data_input}_mult_reconstruct_r${4}_n${1}.log
${fm},0,F,F,F,F,0,${FBEAUT},${FFILT},F,F,0,T,0,1		!CFORM,IFLAG,FMAG,FDEF,FASTIG,FPART,IEWALD,FBEAUT,FFILT,FBFACT,FMATCH,IFSC,FDUMP,IMEM,INTERP
${ro},${ri},${pix},${MW},${AmpC},0.0,${pbc},0.0,10.,1,10,0.0,0.0,1.0  !RO,RI,PSIZE,WGH,XSTD,PBC,BOFF,DANG,ITMAX,IPMAX,SIGANG,SIGSHF,FRAC
1,1,1,1,1,${DMASK}					!MASK
${1},${2}						!IFIRST,ILAST 
${sym}							!ASYM symmetry card (I=icosahedral)
1.,${dstep},60.0,${thresh},${cs},${kV},${tx},${ty}	!RELMAG,DSTEP,TARGET,THRESH,CS,AKV,TX,TY
${rrec}, 200.0, ${rref}, ${rclas}, 100.0, ${rbf}	!RREC,RMIN,RMAX,RCLAS,DFSTD,RBFACT
${raw_images_file}
/dev/null
${working_directory}/${data_input}_${start}_r${4}.par
${data_input}_${start}_r${4}_n${1}.res
...
eot
#
# mv ${data_input}_${start}_r${4}.${extension} ${working_directory}/.
\rm ${data_input}_${start}_r${4}_weights
\rm ${data_input}_${start}_r${4}_n${1}.shft
\rm ${data_input}_${start}_r${4}_map1.${extension}
\rm ${data_input}_${start}_r${4}_map2.${extension}
