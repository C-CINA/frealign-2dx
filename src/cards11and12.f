...
C**************************************************************************
	SUBROUTINE CARDS11AND12(NDOC1,NDOC2,NSET,CDATE,CTIME,
     ...
     .  INTERP,RCLAS,FOUTPAR1,SIGANG,SIGSHF,FRAC)
C**************************************************************************
...
6718		 FORMAT('C Angular step size for search . . . . ',F10.1)
     ...
     +	      PSIZE/RCLAS(NSET),SIGANG,SIGSHF,FRAC,DFSTD(NSET),
     +        RBFAC(NSET),FINPAT1(NSET)(1:SLEN2(FINPAT1(NSET)))
	CALL FLUSH(NDOC1+NSET)
6701	FORMAT(  'C First particle . . . . . . . . . . . ',I10,
     ...
     +          /'C Angular change STD [degrees]. . . . .',F11.3,
     +          /'C Translational change STD [Angstroms].',F11.3,
     +          /'C Fraction of particles to refine. . . ',F11.3,  
