...
C -------------------------------------------------------------------
     ...
     +  FFTW_PLANS,NONSYM,ISM,SM,IREDUN,SIGRAD,SIGPHA,FRAC)
C ---------------------------------------------------------------------
#ifdef _NAG
        USE F90_UNIX
#endif
...
C           PRINT *,'LMAIN SIGANG: ',SIGANG
C           PRINT *,'LMAIN SIGSHF: ',SIGSHF
C
          IF (NONSYM) THEN
            K=ISM+(KK-1)*IREDUN
          ELSE
...
6012        FORMAT(' Refine magnification for particles',I7,' to',I7,
     .             '  on film',I6,'     (K=',I7,')')
            CALL FLUSH(6)
            CALL MAGREFINE(AMAGP,DFMID1,DFMID2,ANGAST,
     +			IFIRSTF,LASTF,NPAR,NSAM,MAXR1,MAXR2,PMASK,C3DF,IRADA,PBUF,
...
6011        FORMAT(' Refine defocus for particles',I7,' to',I7,
     ...
     +		LASTF,FASTIG,NPAR,NSAM,MAXR1,MAXR2,PMASK,DDFMID1,DDFMID2,
     +	        DANGAST,C3DF,IRADA,PBUF,SHX,SHY,IPAD,
...
C
     ...
     +        SIG2N,DFSTD(NSET),ASYM,PWEIGHTS,PSSNR,FFTW_PLANS,
     +        SM,SIGRAD,SIGPHA)
...
C
     ...
     +                  YM(NSET)-SHY(K),SX(NSET),SY(NSET))
            ...
            IF (ABS(PSI(K)).GT.2.0*PI) PSI(K)=MOD(PSI(K),2.0*PI)
            IF (ABS(THETA(K)).GT.2.0*PI) THETA(K)=MOD(THETA(K),2.0*PI)
            IF (ABS(PHI(K)).GT.2.0*PI) PHI(K)=MOD(PHI(K),2.0*PI)
            ANGAST(K)=ANGAST(K)-NINT(ANGAST(K)/PI)*pi
            PSX=PSIZE*ABS(AMAGP(K))
