...
C*
C**************************************************************************
      SUBROUTINE MASK3D_MERGE(NSAM,A3DV,RIO,RIC,RMASK,HALFW,FMASK,
     +                  AVE,STD)
C**************************************************************************
C Applies spherical mask with outer radius RIO, inner radius RIC, and
C cosine edge. Center is defined by RMASK, same conventions as for DMASK.
C Used in merge_3d.
C**************************************************************************
C Created by Ricardo Righetto
...
C**************************************************************************
      ...
      RI5=RIC**2
C
C       RM=MIN(RI,FLOAT(NSAM/2)-1.0-HALFW/2.0)
      RM=RI
      RII=RM+HALFW/2.0
      RIH=RM-HALFW/2.0
      ...
      BG=0.0
      NBG=0
      XC=1.0 + 0.5*FLOAT(NSAM) + RMASK(1)
      YC=1.0 + 0.5*FLOAT(NSAM) + RMASK(2)
      ZC=1.0 + 0.5*FLOAT(NSAM) + RMASK(3)
      SUM1=0.0
      SUM2=0.0
      SUM3=0.0
      NA=0
