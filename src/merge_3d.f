...
C
      ...
      INTEGER IPAD,NNSTAT,IFSC,IERR,SLEN2,I,N,NSAM,ISTP,J
...
20    CONTINUE
     ...
     +  ' Output res table (will be appended to existing file)?'
      ...
      WRITE(*,*)' Center of focused reconstruction?'
      READ(*,'(A)')INLINE
      READ(INLINE,*,ERR=99,END=99) (RMASK(J),J=1,3)
      WRITE(*,17023) (RMASK(J),J=1,3)
17023 FORMAT(3F8.2)
      DO 6901 J=1,3
          IF(RMASK(J).LE.0.0) STOP ' invalid RMASK values'
C          DMASK(J)=-1.0
6901  CONTINUE
      GOTO 98
99    WRITE(*,*) 'CARD 3: No RMASK parameters; using default values'
      RMASK(1)=-1.0
98    CONTINUE
      WRITE(*,4010)
...
C       PRINT *,'RMASK(1)=',RMASK(1)
C       PRINT *,'RMASK(2)=',RMASK(2)
C       PRINT *,'RMASK(3)=',RMASK(3)
C       PRINT *,'PSIZE, NSAM=',PSIZE,NSAM
      IF (RMASK(1).GE.0.0) THEN
        RMASK(1)=RMASK(1)/PSIZE-NSAM/2
        RMASK(2)=RMASK(2)/PSIZE-NSAM/2
        RMASK(3)=RMASK(3)/PSIZE-NSAM/2
      ELSE
        RMASK(1)=0.0
        RMASK(2)=0.0
        RMASK(3)=0.0
...
!$OMP SECTION
     ...
     +                  AFMASK,MAVE,MSTD)
        ELSE
          CALL MASK3D_MERGE(NSAM,B3DF,ABS(RI),RIC,RMASK,HALFW,
     +                AFMASK,MAVE,MSTD)
        ENDIF
!$OMP SECTION
     ...
     +                  DUMMY,DUMMY,DUMMY)
        ELSE
          CALL MASK3D_MERGE(NSAM,C3DF,ABS(RI),RIC,RMASK,HALFW,
     +                  DUMMY,DUMMY,DUMMY)
        ENDIF
...
C
     ...
     +                  MAVE,MSTD)
        ELSE
          CALL MASK3D_MERGE(NSAM,A3DF,RI,RIC,RMASK,HALFW,AFMASK,
     +                MAVE,MSTD)
        ENDIF
      ENDIF
C
...
C
      IF (IFSC.EQ.0) THEN
	CALL OPRESSTATHALF_MERGE(NDOC1,VSN,VN,RAWBINS,PF,PR,ISTP,NSTP,
     .                   NSAM,RLIM,PSIZE,FSC,QF,QC,NPIX,NPIXT,CDATE,
