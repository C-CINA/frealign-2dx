...
C
C******************************************************************************
C Regularization for local searches implemented by Ricardo Righetto
C University of Basel, 07/11/2016
C******************************************************************************
#ifdef _NAG
        USE F90_UNIX
#endif
...
CNIKO       MAXSET=100 does not work on some machines....  set back to 20
        ...
        REAL SIGANG,SIGSHF,SIGRAD,SIGPHA
        REAL FRAC
...
C
	...
	CALL CARD2(RI,RIC,PSIZE,MW,WGH,XSTD,PBC,BOFF,DANGIN,IPMAX,
     +             ITMAX,ITMAXA,IPMAXX,SIGANG,SIGSHF,FRAC)
        CALL FLUSH(6)
...
C           PRINT *,'FREALIGN SIGANG: ',SIGANG
C           PRINT *,'FREALIGN SIGSHF: ',SIGSHF
C     Convert the input sigma from degrees to radians:
      SIGRAD=SIGANG*PI/180.0

C     Convert the input sigma from Angstroms to radians (phase shift):
      SIGPHA=SIGSHF*2.0*PI/(NSAM*PSIZE)
...
C       
     ...
     +      RCLAS,DANGIN,FFTW_PLANS,NONSYM,ISM,SM,IREDUN,SIGRAD,SIGPHA,
     +      FRAC)
C
